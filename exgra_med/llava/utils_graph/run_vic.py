...
def get_arguments():
    ...
    #### data parameters ####
    #########################
    parser.add_argument("--log_dir", type=str, default=  "/home/caduser/HDD/hoang_graph_matching/log/", help="path to tensorboard folder")
    parser.add_argument("--tensorboard_name", type=str, default= "bbgm_vic", help="name of tensorboard folder")
    ...
    parser.add_argument("--num_image_per_graph", default= 10, type=int,
        ...
        for step, inputs in enumerate(train_loader):
            ...
            if args.fp16:
                with torch.cuda.amp.autocast():
                    loss, global_loss, local_loss, classif_loss = model.forward(make_inputs(inputs, gpu))
...
class VICRegL(nn.Module):
    def __init__(self, args):
        ...
        self.backbone, self.representation_dim = resnet50(
    ...
    def _local_loss(
        ...
        perm_matrix = torch.diag(torch.ones(self.num_image_per_graph),0).unsqueeze(0).to(device) 
        perm_matrix_1 = torch.diag(torch.ones(redundant_images),0).unsqueeze(0).to(device) 
   
        prediction_list = [] 
        target_list = []
        losses = 0.0
        for index in range(num_graphs):
            sub_node_1 = maps_1[index * self.num_image_per_graph : (index + 1) * self.num_image_per_graph].float()
            sub_node_2 = maps_2[index * self.num_image_per_graph : (index + 1) * self.num_image_per_graph].float()
            # build graph
            orig_graph = self.build_graph(sub_node_1, sub_node_2)
            # cal local feature to c^v
            sub_maps_1_filtered_l2 = maps_1_filtered_l2[index * self.num_image_per_graph :                                                (index + 1) * self.num_image_per_graph].float()
            sub_maps_2_nn_l2 = maps_2_filtered_l2[index * self.num_image_per_graph : 
(index + 1) * self.num_image_per_graph].float()
            sub_maps_1_filtered_location = maps_1_filtered_location[index * self.num_image_per_graph : 
(index + 1) * self.num_image_per_graph].float()
            sub_maps_2_nn_location = maps_2_filtered_location[index * self.num_image_per_graph : 
(index + 1) * self.num_image_per_graph].float()
            unary_costs_list = [
            ...
            unary_costs_list[0] += (
                                   ...
                                   self._l2_distance(sub_maps_1_filtered_location,sub_maps_2_filtered_location) 
            )
    ...
    def build_graph(self, global_map1, global_map2):
        ...
        edge_indice1, edge_feature1 = build_graphs_main_1(global_map1, n_neighbors = self.num_neighbors, num_workers = 1)
        edge_indice2, edge_feature2 = build_graphs_main_1(global_map2, n_neighbors = self.num_neighbors, num_workers = 1)
        data_graph1 = Data(x = global_map1, edge_index = edge_indice1, edge_attr = edge_feature1)
        data_graph2 = Data(x = global_map2, edge_index = edge_indice2, edge_attr = edge_feature2)
    ...
    def loss_calculate(self, embedding):
        ...
        for index in range(num_graphs):
            ...
            start = time.time()
            sub_node_view1 = embedding[0][index * self.num_image_per_graph : (index + 1) * self.num_image_per_graph].float()
            sub_node_view2 = embedding[1][index * self.num_image_per_graph : (index + 1) * self.num_image_per_graph].float()
...
class GDPModel1(torch.nn.Module):
    def __init__(self, num_features=512, hidden_size=512, edge_dim = 512):
        ...
        convs = [GraphConv(self.num_features, self.hidden_size),
