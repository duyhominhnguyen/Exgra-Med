    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(
                input_ids
            )  # [B, T, hidden_size] e.g [4,329,4096], E in CG-VLM
        ...
        if (
            ...
            with torch.no_grad():
                if type(images) is list:
                    ...
                    image_features = []
                    for image in images:
                        image_feature = self.extract_visual_features(
            ...
            ):  # loop through each conversation in a batch, cur_input_embeds: [329, 4096]
        ...
        return (
            super(LlavaLlamaModel, self).forward(
                input_ids=None,
...
class LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        hidden_states = outputs[
            0
        ]  # size [B, T, config.hidden_size] = (e.g) [4,329, 4096], T is length of longest sequence in the batch
