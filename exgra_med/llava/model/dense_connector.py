...
def dense_connector_sti(image_features, image_forward_outs, is_siglip=False):
    ...
    if not is_siglip:
        image_features_1 = image_forward_outs.hidden_states[7][:, 1:].to(
            image_features.dtype
        )
        image_features_2 = image_forward_outs.hidden_states[15][:, 1:].to(
            image_features.dtype
        )
    else:
        image_features_1 = image_forward_outs.hidden_states[7][:, :].to(
            image_features.dtype
        )
        image_features_2 = image_forward_outs.hidden_states[15][:, :].to(
            image_features.dtype
        )
    image_features_1 = avg_pooling_k8(image_features_1.permute(0, 2, 1)).permute(
        0, 2, 1
    )
    image_features_2 = avg_pooling_k8(image_features_2.permute(0, 2, 1)).permute(
        0, 2, 1
    )
...
def dense_connector_dci(
    image_features, image_forward_outs, is_siglip=False, is_biomed=False, num_l=6
):
    image_features_1 = []
    image_features_2 = []
    if is_biomed:
        print(
            "================== num element for DCI: {}-{} =============".format(
                num_l, num_l
            )
        )
        ...
        for i in range(0, 13):
            image_features_1.append(
                image_forward_outs.hidden_states[i][:, :].to(image_features.dtype)
            )
        image_features_1 = torch.stack(image_features_1, dim=0)
...
def dense_connector(
    ...
    if mm_dense_connector_type == "sti":
        image_features_dc = dense_connector_sti(
            image_features, image_forward_outs, is_siglip
        )
        image_features = torch.cat((image_features, image_features_dc), dim=-2)
    elif mm_dense_connector_type == "sci":
        image_features_dc = dense_connector_sci(
            image_features, image_forward_outs, is_siglip
        )
        image_features = torch.cat((image_features, image_features_dc), dim=-1)
    elif mm_dense_connector_type == "dci":
        image_features_dc = dense_connector_dci(
            image_features, image_forward_outs, is_siglip, is_biomed, num_l
        )
        image_features = torch.cat((image_features, image_features_dc), dim=-1)
    else:
