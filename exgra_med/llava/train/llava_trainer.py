...
class LLaVATrainer(Trainer):
        ...
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            ...
            for k, v in _state_dict.items():
                if any(key_match in k for key_match in keys_to_match):
                    weight_to_save[k] = (
                        v.cpu().clone().detach()
                    )  # Chunyuan: to solve the saving OOM problem
    ...
    def prediction_step(
        ...
        loss, _, labels = super(LLaVATrainer, self).prediction_step(
            model,
            inputs,
        ...
        im_end_token_pos_list = (inputs["input_ids"] == im_end_id).nonzero()[
            :, 1
        ]  # tensor of all positions where image tokens end.
        ...
        for im_end_token_pos, conversation, image, text_input in zip(
            im_end_token_pos_list,
            inputs["input_ids"],
            inputs["images"],
            inputs["text_input"],
        ):
            prompt = (
                conversation[: im_end_token_pos + 2].unsqueeze(0).to(self.args.device)
            )  # [1,T]
            image = image.unsqueeze(0).to(dtype=dtype, device=self.args.device)
            keywords = ["###"]
            stopping_criteria = KeywordsStoppingCriteria(
                keywords, self.tokenizer, prompt
            )
            ...
            with torch.inference_mode():
                output_ids = model.generate(
                    ...
                    synced_gpus=True,
                )  # [1,T], output_ids also include prompt

            input_token_len = prompt.shape[1]
            n_diff_input_output = (
                (prompt != output_ids[:, :input_token_len]).sum().item()
            )
            if n_diff_input_output > 0:
                print(
                    f"[Warning] Sample: {n_diff_input_output} output_ids are not the same as the input_ids"
                )
            # outputs = self.tokenizer.batch_decode(output_ids[:, input_token_len:], skip_special_tokens=True)[0]
            # print(outputs)
            generated_tokens_ids.append(output_ids[:, input_token_len:].squeeze())
            pbar.update(1)
        generated_tokens_ids = torch.nn.utils.rnn.pad_sequence(
            generated_tokens_ids,
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id,
        )  # size [B, T1] with T1 is length of longest sequence
        pbar.close()
