...
conv_to_str = lambda conv: "\n\n".join(
    [("User: " if x["from"] == "human" else "Assistant: ") + x["value"] for x in conv]
)
  ...
  """,
        ...
        for ex in instruct_few_shot_examples.fs:
            messages += [
                {
                    "role": "user",
                    "content": PromptGenerator.context_gen(ex, use_inline_mentions),
                },
    ...
    def wrap_gen_message(sample, use_inline_mentions=False):
        text = PromptGenerator.context_gen(
            sample, use_inline_mentions=use_inline_mentions
        )
        context = PromptGenerator.few_shot_messages_gen(
            text, use_inline_mentions=use_inline_mentions
        )
        return context
...
def main(args):
    ...
    for i in range(3):
        ...
        batch = []
        counter = 0
        for cycle_idx, samples in enumerate(
            itertools.zip_longest(*domain_dict.values())
        ):
            ...
            for domain_idx, sample in enumerate(samples):
                ...
                if len(batch) >= args.batch_size:
                    async_results = call_async(
                        batch,
                        lambda x: PromptGenerator.wrap_gen_message(
                            x, use_inline_mentions=args.use_inline_mentions
                        ),
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path",
        type=str,
        default="data/instruct/llava_med_instruct_fig_captions.json",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="data/instruct/llava_med_instruct_fig_captions_gen.json",
    )
