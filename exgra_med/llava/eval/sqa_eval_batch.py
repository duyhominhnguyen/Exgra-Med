...
def parse_args():
    ...
    parser.add_argument(
        "--model-name", type=str, required=True, help="Path to the model checkpoint."
    )
    parser.add_argument(
        "--science-qa-path",
        type=str,
        required=True,
        help="Path to the ScienceQA base folder.",
    )
    parser.add_argument(
        "--chunks", type=int, default=8, help="Number of chunks (default: 8)."
    )
    parser.add_argument(
        "--experiment-name",
        type=str,
    ...
    parser.add_argument(
        "--split",
        type=str,
        choices=["minival", "val", "minitest", "test"],
        required=True,
        help="Dataset split to evaluate (choices: minival, val, minitest, test).",
    )
...
def run_job(chunk_idx, args):
    question_file = os.path.join(
        args.science_qa_path, f"data/scienceqa/llava_{args.split}_QCM-LEPA.json"
    )
    ...
    image_folder = os.path.join(
        args.science_qa_path, f"data/scienceqa/images/{image_split}"
    )

    cmd = (
        "CUDA_VISIBLE_DEVICES={chunk_idx} python -m llava.eval.model_vqa_science "
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=args.chunks) as executor:
        list(
            executor.map(run_job_with_args, range(args.chunks))
        )  # Use run_job_with_args instead of lambda
    ...
    with open(output_file, "w") as outfile:
        for idx in range(args.chunks):
            with open(
                f"./{args.experiment_name_with_split}-chunk{idx}.jsonl"
            ) as infile:
