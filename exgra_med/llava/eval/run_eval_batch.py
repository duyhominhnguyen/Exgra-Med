...
def parse_option():
    ...
    parser.add_argument(
        "--target_test_type",
        type=str,
        default="test-answer-file",
        help="[test-answer-file, test_with_options-answer-file]",
    )
...
def evaluate(gt, pred, candidate, criterion=None):
        ...
        if gt_item["answer_type"] == "OPEN":
            ...
            if gt_value in pred_value:
                ...
                open_hit_scores["hit"].append(
                    0
                )  # IMPORTANT: we are missing this line before; skip it is dangerous
            ...
            f1_score_closed, precision_closed, recall_closed = calculate_f1score(
                pred_value, gt_value
            )
    ...
    open_hit_score = (
        sum(open_hit_scores["hit"]) / len(open_hit_scores["hit"])
        if len(open_hit_scores["hit"]) != 0
        else 0.0
    )
    closed_score = (
        ...
        for ds in dataset_list:
            if ds in f:
                ...
                try:
                    ...
                    assert (
                        gt_ids == pred_ids
                    ), "please make sure pred and gt are exactly matched"

                    # perform evaluation
                    results = evaluate(gt, pred, candidate)

                    ans_file.write(
                        json.dumps({"dataset": ds, "pred_file": f, "results": results})
                        + "\n"
                    )
                    ans_file.flush()
                    print(results)
