        ...
        for sample in samples:
            ...
            if len(batch) >= BATCH_SIZE:
                async_results = call_async(
                    batch, lambda x: LLMEvalPromptGenerator.compare_messages_gen(x)
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--question_input_path",
        type=str,
        default="data/eval/llava_med_eval_qa50_qa.jsonl",
    )
    parser.add_argument(
        "--input_path",
        type=str,
        default="dbfs:/mnt/hanoverdev/scratch/clwon/llava/test/answers/test50/2023-05-10_med-pretrain-364m-v1-1epoch.jsonl",
    )
    parser.add_argument(
        "--output_path", type=str, default="data/eval/llava_med_eval_qa50_qa_ans.jsonl"
    )
