...
def parse_args():
    ...
    parser.add_argument(
        "--num-chunks", type=int, default=1, help="Number of chunks (default: 1)."
    )
...
def run_job(chunk_idx, args):
    ...
    cmd = (
        "CUDA_VISIBLE_DEVICES={chunk_idx} python llava/eval/model_vqa_med.py "
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=args.num_chunks) as executor:
        list(
            executor.map(run_job_with_args, range(args.num_chunks))
