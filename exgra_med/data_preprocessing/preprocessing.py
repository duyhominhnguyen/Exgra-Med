...
def parse_option():
    ...
    parser.add_argument(
        "--dataset",
        choices=["vqa-rad", "data_RAD", "slake", "pvqa"],
        default="test",
    ...
    parser.add_argument(
        ...
        default="test",
        help="Processing test or train dataset",
    )
    ...
    for vqa in data:
        ...
        if vqa["answer_type"] == "CLOSED":
            if vqa["answer"].lower() in ["yes", "no"]:
                ...
                print(
                    "'{}' is not a question with answers [yes,no] but {}".format(
                        vqa["question"], vqa["answer"]
                    )
                )
                print(
                    "Please type each candidate answers here. Enter new line after each candidate. Ctrl-D or Ctrl-Z ( windows ) to save it."
                )
                contents = []
                while True:
                    try:
                ...
                candidate_answers = " (Answer with either {}).".format(
                    " or ".join(f'"{w}"' for w in contents)
                )
        ...
        vqa_instruct["conversations"] = [
            ...
            {"from": "gpt", "value": vqa["answer"]},
        ]
        content.append(vqa_instruct)
    file_name = (
        "test_w_options_new.json" if "test" in args.path else "train_w_options_new.json"
    )
...
def process_pvqa():
    """
    Process PathVQA dataset to conversation-style format.
    Args:
        args.path: path to the raw PathVQA data file (pickle format), e.g ./train_vqa.pkl
    """
    ...
    for vqa in data:
        ...
        vqa_instruct["answer_type"] = (
            "CLOSED" if vqa["answer_type"] == "yes/no" else "OPEN"
        )
        vqa_instruct["conversations"] = [
            {"from": "human", "value": vqa["sent"] + candidate_answers + "\n<image>"},
            {"from": "gpt", "value": next(iter(vqa["label"]))},
        ]
        content.append(vqa_instruct)
    file_name = args.split + "_w_options_new.json"
    with open(file_name, "w") as f_saved:
...
if args.dataset == "data_RAD":
    ...
    raise ValueError(
        "Unknown data, please choose from the following options: ['vqa-rad', 'data_RAD', 'slake', 'pathvqa']"
    )
